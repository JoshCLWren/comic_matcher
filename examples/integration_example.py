...
def comic_pricer_integration(web_pricer_path, comic_pricer_path, output_path):
    ...
    if not matches.empty:
        ...
        for _, match in matches.head(5).iterrows():
            print(
                f"  {match['source_title']} #{match['source_issue']} = {match['target_title']} #{match['target_issue']} (similarity: {match['similarity']:.2f})"
            )
    else:
        print("\nNo matches found")
...
def xmen_reading_order_integration(reading_order_path, wishlist_path, output_path):
    ...
    if not matches.empty:
        ...
        for _, match in unread_matches.head(5).iterrows():
            print(
                f"  {match['source_title']} #{match['source_issue']} (Order: {match['order']}) = {match['target_title']} #{match['target_issue']} (${match.get('price', 'N/A')})"
            )
    else:
        print("\nNo matches found")
...
def main():
    ...
    if any(
        not os.path.exists(os.path.join(os.path.dirname(__file__), p))
        for p in SAMPLE_PATHS.values()
    ):
        print(
            "Sample data files not found. This script is an example of how to integrate with your existing data."
        )
