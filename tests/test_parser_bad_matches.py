...
class TestParserBadMatches:
    ...
    def test_parser_extracts_subtitle_correctly(self):
        ...
        for title, expected in test_cases:
            ...
            assert result["main_title"] == expected, (
                f"Expected main_title '{expected}' for '{title}', "
                f"but got '{result['main_title']}'"
            )
    ...
    def test_parser_handles_annual_correctly(self):
        ...
        for title, expected in test_cases:
            ...
            assert result["special"] == expected, (
                f"Expected special '{expected}' for '{title}', "
                f"but got '{result['special']}'"
            )
    ...
    def test_parser_handles_series_versions(self):
        """Test parsing of series versions (e.g., X-Men Forever 2)"""
        parser = ComicTitleParser()
        
        # This test checks if the volume info is correctly extracted
        test_cases = [
            # Format: title, expected_volume
            ("X-Men Forever 2", "2"),  # Should extract "2" as volume
            ("Civil War II", ""),  # Roman numerals aren't extracted as volume
            ("New X-Men Vol 2", "2"),
            ("New X-Men Volume 2", "2"),
        ]
        
        for title, expected in test_cases:
            result = parser.parse(title)
            
            # For titles like "X-Men Forever 2", we would ideally want to recognize "2" as a version
            # or volume, but the current parser may not handle this specific case
            if expected:
                assert result["volume"] == expected, (
                    f"Expected volume '{expected}' for '{title}', "
                    f"but got '{result['volume']}'"
                )
    ...
    def test_parse_issue_number_from_title(self):
        ...
        for title, expected in test_cases:
            ...
            assert issue == expected, (
                f"Expected issue '{expected}' for '{title}', "
                f"but got '{issue}'"
            )
    ...
    def test_extract_issue_number_special_cases(self):
        ...
        test_cases = [
            ...
            ("Annual 2000", None),  # Annual with year
            ("#1", "1"),  # Standard format
        ...
        for issue_text, expected in test_cases:
            ...
            assert issue == expected, (
                f"Expected issue '{expected}' for '{issue_text}', "
                f"but got '{issue}'"
            )
    ...
    def test_annual_vs_issue_parsing(self):
        ...
        assert result2["special"] == "annual", (
            f"'X-Men Annual 2000' should have 'annual' as 'special', "
            f"but got '{result2['special']}'"
        )
        ...
        assert issue1 != issue2, (
            f"Issue numbers '2000' and '1' should be different, "
            f"but got '{issue1}' and '{issue2}'"
        )
