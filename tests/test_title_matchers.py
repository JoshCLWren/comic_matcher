...
def test_title_comparison_directly():
    ...
    for source_title, target_title, expected in KEY_MATCH_CASES:
        ...
        if expected:
            assert similarity >= 0.7, f"Similarity too low: {similarity} for '{source_title}' and '{target_title}'"
        else:
            assert similarity < 0.7, f"Similarity too high: {similarity} for '{source_title}' and '{target_title}'"
...
def test_batch_matching():
    ...
    for source, target, expected in KEY_MATCH_CASES:
        if expected and (source, target) not in matched_pairs and not any(
