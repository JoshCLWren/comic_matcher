...
class TestUtils:
    ...
    @patch("pathlib.Path.open", new_callable=mock_open, read_data='{"comics": [{"title": "X-Men", "issue": "1"}]}')
    def test_load_comics_from_json(self, mock_file):
        ...
        with patch("pathlib.Path.open", new_callable=mock_open, read_data='[{"title": "X-Men", "issue": "1"}]'):
            result = load_comics_from_json("list_path.json")
            assert isinstance(result, list)
            assert len(result) == 1
        
        # Test with results key
        with patch("pathlib.Path.open", new_callable=mock_open, read_data='{"results": [{"title": "X-Men", "issue": "1"}]}'):
            result = load_comics_from_json("results_path.json")
            assert isinstance(result, list)
            assert len(result) == 1
        
        # Test with flat dictionary
        with patch("pathlib.Path.open", new_callable=mock_open, read_data='{"1": {"title": "X-Men", "issue": "1"}}'):
